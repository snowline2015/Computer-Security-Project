...
def sign_file(filename, Kprivate, password):
    ...
    signature = pow(int.from_bytes(hash, 'big'), int(d), int(n))
...
def verify_file(filename, Kpublic):
    ...
    hash = pow(signature, int(e), int(n))
    return hash == int.from_bytes(hashlib.sha256(open(filename, 'rb').read()).digest(), 'big')
...
#haha = sign_file("D:\\ASUS\\Videos\\Favorites\\Kabaneri of The Iron Fortress OST - KABANERIOFTHEIRONFORTRESS (Feat. Eliana).mp4", (e, n))
sign_file("D:\\ASUS\\Videos\\Favorites\\Kabaneri of The Iron Fortress OST - KABANERIOFTHEIRONFORTRESS (Feat. Eliana).mp4", (d, n), "residentevil4567")
haha = verify_file("D:\\ASUS\\Videos\\Favorites\\Kabaneri of The Iron Fortress OST - KABANERIOFTHEIRONFORTRESS (Feat. Eliana).mp4", (e, n))
